...
SplitFilterGraph::SplitFilterGraph(
        const AVCodecContext& source_codec_context,
    ...
    /*
     * connect filters
     * each output of channelsplit -> format -> setnsamples -> sink
     */
...
SplitFilterGraph::~SplitFilterGraph() {
    /* this also frees all filters */
    avfilter_graph_free(&m_graph);
}

int SplitFilterGraph::send_frame(AVFrame* frame) {
    return av_buffersrc_add_frame(m_src_context, frame);
}

int SplitFilterGraph::receive_sink_frame(AVFrame* frame, int sink) {
